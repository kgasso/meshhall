...
| `!net start <slug>` | DM/channel | Manually open a session; also: `!net open` (net control/admin) |
| `!net stop <slug>` | DM/channel | Manually close a session; also: `!net close` (net control/admin) |
| `!net edit <slug> <field> <value>` | DM | Edit net settings one field at a time (net control/admin) |
| `!net create <slug> <n>` | DM | Create a new net; use `!net edit` to configure (admin) |
...
**`!net edit` fields:** `name`, `schedule`, `timezone`, `duration`, `channel`, `guests`, `description`

Use `none` to clear `schedule`, `channel`, or `description`. Schedule examples: `weekly tuesday 19:00`, `monthly 3rd tuesday 19:00`, `daily 08:00`
...
Before running MeshHall, your MeshCore radio node needs to be configured
correctly. These steps use the MeshCore Android/iOS companion app.

### Step 1: Open Settings

Log into your node via the companion app. Tap the gear icon at the top right
of the interface to open the Settings panel.

![MeshCore main interface showing the gear icon](docs/images/settings_0_main.png)

### Step 2: Validate Radio Settings and Open Contact Settings

Confirm your radio settings are compliant with your regional radio regulations
and compatible with the mesh you intend to join (frequency, bandwidth,
spreading factor, and coding rate must match the network). Once confirmed, tap
**Contact Settings** to open the contact management panel.

![Settings panel showing Radio Settings and Contact Settings](docs/images/settings_1_radio-contact.png)

### Step 3: Configure Auto-Add Behavior

Set Auto-Add to **Auto Add Selected** and enable only **Auto Add Chat Users**.
Disable Auto Add Repeaters, Auto Add Room Servers, and Auto Add Sensors. This
prevents the bot's contact list from filling up with infrastructure nodes,
reserving capacity for chat users. Enable **Overwrite Oldest** so that when
the contact list is full, the oldest non-favourite contacts are replaced by
newly seen nodes.

![Contact Settings panel with Auto Add Chat Users selected](docs/images/settings_2_contact.png)
...
### 4. Configure your radio node

Before starting the bot, configure the RAK4631 (or other MeshCore node) for
automatic contact management:

**Contact Settings -> Auto Add Contacts**
Enable auto-add for at minimum **Chat Users**. This allows the bot to receive
DMs from nodes that have advertised -- without this, direct messages will never
reach the bot.

> If you want to test the bot's contact pruning behaviour, you can also enable
> auto-add for Repeaters, Room Servers, and Sensors. The bot will log the
> contact `type` field for each advertisement so you can confirm the type
> values before enabling type-based prune filtering.
...
plugins/
  01_time.py                 !time
  02_nets.py                 !net * subcommands: list info checkin regrets roll start stop open close edit create delete add remove promote grant revoke

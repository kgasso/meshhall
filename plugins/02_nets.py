...
def setup(dispatcher, config, db):
    ...
    async def do_create(msg, args=""):
        """
        !net create <slug> <name>
        Creates a net with defaults. Use !net edit to configure schedule,
        timezone, duration, channel, guests, and description afterward.
        """
        raw = args.strip()
        if not raw:
            return (
                "Usage: !net create <slug> <name>\n"
                "  slug -- lowercase, letters/numbers/hyphens, max 32 chars\n"
                "  name -- display name (may contain spaces)\n"
                "Use !net edit <slug> <field> <value> to configure further."
            )

        tokens = raw.split(None, 1)
        if len(tokens) < 2:
            return "Usage: !net create <slug> <name>"
        ...
        await db.execute(
            """INSERT INTO nets
               ...
               VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
            (slug, name, None, None, 0, tz_str, 60, None, None,
             msg.sender_id, _now()),
        )
        await db.commit()
        return (
            f"Net '{slug}' ({name}) created (no schedule, ad-hoc).\n"
            f"Use !net edit {slug} <field> <value> to configure."
        )
    ...
    async def do_delete(msg, args=""):
        ...
        dispatcher.log_admin_attempt("!net delete", msg, granted=True, reason=f"deactivating net '{slug}'")
    ...
    async def do_add(msg, args=""):
        ...
        dispatcher.log_admin_attempt("!net add", msg, granted=True, reason=f"adding {user['pubkey_prefix']} to {net['name']}")
        await db.execute(
    ...
    async def do_remove(msg, args=""):
        ...
        dispatcher.log_admin_attempt("!net remove", msg, granted=True, reason=f"removing {user['pubkey_prefix']} from {net['name']}")
        await db.execute(
    ...
    async def do_promote(msg, args=""):
        ...
        dispatcher.log_admin_attempt("!net promote", msg, granted=True, reason=f"promoting {user['pubkey_prefix']} to full member of {net['name']}")
        await db.execute(
    ...
    async def do_grant(msg, args=""):
        ...
        dispatcher.log_admin_attempt("!net grant", msg, granted=True, reason=f"granting NC to {user['pubkey_prefix']} on {net['name']}")
        await db.execute(
    ...
    async def do_revoke(msg, args=""):
        ...
        dispatcher.log_admin_attempt("!net revoke", msg, granted=True, reason=f"revoking NC from {user['pubkey_prefix']} on {net['name']}")
        await db.execute(
    ...
    async def do_start(msg, args=""):
        ...
        dispatcher.log_admin_attempt("!net start", msg, granted=True, reason=f"opening session for {net['name']}")
        sess_id = await _open_session(net)
        cc = dispatcher.command_char
        await _announce(
            net,
    ...
    async def do_stop(msg, args=""):
        ...
        dispatcher.log_admin_attempt("!net stop", msg, granted=True, reason=f"closing session for {net['name']}")
    ...
    async def do_edit(msg, args=""):
        """
        !net edit <slug> <field> <value>

        Editable fields:
          name        <text>       -- display name
          schedule    <human|none> -- recurrence or 'none' to clear
          timezone    <IANA>       -- e.g. America/Los_Angeles
          duration    <minutes>    -- session length in minutes
          channel     <name|none>  -- bound channel or 'none' to clear
          guests      yes|no       -- allow guest check-ins
          description <text|none>  -- free-form description or 'none' to clear
        """
        cc    = dispatcher.command_char
        usage = (
            f"Usage: {cc}net edit <slug> <field> <value>\n"
            "Fields: name, schedule, timezone, duration, channel, guests, description\n"
            "  schedule none / channel none / description none -- clears the field"
        )
        ...
        _EDITABLE = {"name", "schedule", "timezone", "duration",
                     "channel", "guests", "description"}
        ...
        if field == "name":
            ...
            await db.execute("UPDATE nets SET name=? WHERE id=?", (value, net["id"]))
            await db.commit()
            return f"Net '{slug}' name updated to '{value}'."
            ...
            if value.lower() in ("none", "clear"):
                ...
                await db.commit()
                return f"{net['name']} schedule cleared -- sessions opened manually with !net start."
            try:
                cron_expr, cron_human = parse_schedule(value)
            except ValueError as e:
                return f"Schedule error: {e}"
            await db.execute(
                "UPDATE nets SET cron_expr=?, cron_human=? WHERE id=?",
                (cron_expr, cron_human, net["id"]),
            )
            await db.commit()
            tz_str = net["timezone"] or _default_tz()
            return (
            ...
            try:
                ...
                return f"Unknown timezone '{value}'. Use IANA names e.g. America/Los_Angeles"
            await db.execute("UPDATE nets SET timezone=? WHERE id=?", (value, net["id"]))
            await db.commit()
            tz_note = ""
            if net["cron_expr"]:
                tz_note = f"\nNext: {_fmt_next(net['cron_expr'], value)}"
            return f"{net['name']} timezone updated to '{value}'.{tz_note}"
            ...
            try:
                ...
                return "Duration must be a positive integer number of minutes."
            await db.execute("UPDATE nets SET duration_min=? WHERE id=?",
                             (minutes, net["id"]))
            await db.commit()
            return f"{net['name']} duration updated to {minutes} minutes."

        elif field == "channel":
            if value.lower() in ("none", "clear"):
                await db.execute("UPDATE nets SET channel=NULL WHERE id=?", (net["id"],))
                await db.commit()
                return f"{net['name']} channel binding cleared."
            await db.execute("UPDATE nets SET channel=? WHERE id=?",
                             (value, net["id"]))
            await db.commit()
            return f"{net['name']} bound to channel '{value}'."

        elif field == "guests":
            allow = value.lower() in ("yes", "true", "1", "on")
            await db.execute("UPDATE nets SET allow_guests=? WHERE id=?",
                             (1 if allow else 0, net["id"]))
            await db.commit()
            return f"{net['name']} guest check-ins {'enabled' if allow else 'disabled'}."

        elif field == "description":
            if value.lower() in ("none", "clear"):
                await db.execute("UPDATE nets SET description=NULL WHERE id=?",
                                 (net["id"],))
                await db.commit()
                return f"{net['name']} description cleared."
            await db.execute("UPDATE nets SET description=? WHERE id=?",
                             (value, net["id"]))
            await db.commit()
            return f"{net['name']} description updated."

        return usage  # unreachable but safe
    ...
    _SUBCOMMANDS = {
        ...
        "start":       do_start,
        "stop":        do_stop,
        "open":        do_start,   # undocumented alias for start
        "close":       do_stop,    # undocumented alias for stop
        "edit":        do_edit,
        "add":         do_add,
        "remove":      do_remove,
        "promote":     do_promote,
    ...
    _ADMIN_SUBS = {"grant", "revoke", "create", "delete"}
    _NC_SUBS    = {"start", "stop", "open", "close", "edit",
                   "add", "remove", "promote"}
        ...
        if not sub:
            ...
            if is_nc or is_admin:
                lines += [
                    ...
                    f"  {cc}net edit <slug> <field> <value> -- edit net settings",
